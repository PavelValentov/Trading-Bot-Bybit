# BYBIT ТОРГОВЫЙ БОТ - КРАТКОЕ ОПИСАНИЕ ПРОЕКТА

## ОБЗОР ПРОЕКТА
Разработка алгоритмического торгового бота для криптобиржи Bybit с многофакторным анализом на основе технических индикаторов, фундментального анализа новостей и макроэкономических показателей.

## ЦЕЛЬ ПРОЕКТА
Создание автоматизированной торговой системы, способной принимать решения на основе комплексного анализа рыночной ситуации с использованием весовых коэффициентов для различных факторов.

## КЛЮЧЕВЫЕ ОСОБЕННОСТИ
- Торговля на 4-часовом таймфрейме
- Начальные торговые пары: BTC, ETH, SOL
- Интеграция с множественными источниками данных
- Система весовых коэффициентов для факторов
- Возможность ретротестирования стратегий

## ТЕКУЩИЙ СТАТУС
🔴 ПРОЕКТ НА СТАДИИ ИНИЦИАЛИЗАЦИИ
Требуется детальная проработка технического задания с конкретизацией параметров, торговой стратегии и источников данных.

## СЛЕДУЮЩИЕ ШАГИ
1. Создание детального технического задания
2. Проработка торговой стратегии
3. Архитектурное планирование
4. Техническая реализация

## ДАТА СОЗДАНИЯ
2025-08-27 17:10:00

## ТРЕБОВАНИЯ К ДОКУМЕНТИРОВАНИЮ ЗАДАЧ

### ОБЯЗАТЕЛЬНОЕ ВКЛЮЧЕНИЕ FILE REFERENCES
**КРИТИЧЕСКОЕ ТРЕБОВАНИЕ:** Начиная с текущего момента, ВСЕ задачи в мемори-банке должны содержать **полные ссылки на файлы**, которые:

1. **Созданы в рамках задачи**
2. **Модифицированы в рамках задачи** 
3. **Анализируются или используются** в задаче
4. **Влияют на результат** задачи

### ФОРМАТ FILE REFERENCES
Каждая задача должна содержать секцию:

```markdown
## FILE REFERENCES
### Созданные файлы:
- `/path/to/created/file.ext` - описание назначения файла
- `/path/to/another/created/file.ext` - описание назначения файла

### Модифицированные файлы:
- `/path/to/modified/file.ext` - описание внесенных изменений
- `/path/to/another/modified/file.ext` - описание внесенных изменений

### Связанные файлы:
- `/path/to/related/file.ext` - описание связи с задачей
- `/path/to/dependency/file.ext` - описание зависимости
```

### ПРИМЕРЫ ПРАВИЛЬНОГО ДОКУМЕНТИРОВАНИЯ

#### Задача: Создание API эндпоинта
```markdown
## FILE REFERENCES
### Созданные файлы:
- `/src/controllers/trading.controller.ts` - Контроллер для торговых операций
- `/src/dto/trading.dto.ts` - DTO для валидации входных данных
- `/tests/controllers/trading.controller.spec.ts` - Unit тесты для контроллера

### Модифицированные файлы:
- `/src/app.module.ts` - Добавлен TradingModule в imports
- `/package.json` - Добавлены зависимости для валидации
- `/README.md` - Обновлена документация API

### Связанные файлы:
- `/src/services/decision-engine.service.ts` - Используется для принятия торговых решений
- `/src/schemas/trading.schema.ts` - Схема базы данных для торговых операций
```

### ПРИМЕНЕНИЕ К СУЩЕСТВУЮЩИМ ЗАДАЧАМ

**ЗАДАЧИ CREATIVE ФАЗЫ уже содержат file references:**

#### TASK-004: Алгоритм принятия торговых решений
```markdown
## FILE REFERENCES
### Созданные файлы:
- INLINE DOCUMENTATION (в рамках CREATIVE сессии) - Полная спецификация гибридного алгоритма принятия решений
- Планируется: `/src/services/decision-engine.service.ts` - Основная реализация
- Планируется: `/src/modules/decision/decision.module.ts` - Модуль принятия решений
```

#### TASK-005: Система весовых коэффициентов
```markdown
## FILE REFERENCES
### Созданные файлы:
- `/memory-bank/creative/creative-weight-system.md` - Полная спецификация системы весов (3000+ строк)
- Планируется: `/src/services/weight-system.service.ts` - Реализация системы весов
- Планируется: `/src/types/weight-system.types.ts` - Типы и интерфейсы системы весов
```

#### TASK-006: Микросервисная архитектура
```markdown
## FILE REFERENCES
### Созданные файлы:
- `/memory-bank/creative/creative-microservices.md` - Детальная архитектура 15 микросервисов (2500+ строк)
- Планируется: `/docker-compose.yml` - Конфигурация развертывания
- Планируется: `/services/*/package.json` - Конфигурации каждого сервиса
```

#### TASK-007: Схемы баз данных
```markdown
## FILE REFERENCES
### Созданные файлы:
- `/memory-bank/creative/creative-database-schemas.md` - Полные схемы всех БД (2000+ строк)
- Планируется: `/migrations/postgresql/*.sql` - Миграции PostgreSQL
- Планируется: `/schemas/mongodb/*.js` - Схемы MongoDB коллекций
```

## ОБЯЗАТЕЛЬСТВО КОМАНДЫ РАЗРАБОТКИ
Все будущие задачи, начиная с IMPLEMENT фазы, ДОЛЖНЫ содержать полные file references согласно описанному формату.

**Дата внедрения требования:** 2025-08-27 17:10:00

## ЯЗЫКОВАЯ ПОЛИТИКА ПРОЕКТА

### КРИТИЧЕСКОЕ ТРЕБОВАНИЕ К ЯЗЫКАМ ДОКУМЕНТАЦИИ И КОДА
**Дата внедрения:** 2025-08-27 17:10:00

### 🇷🇺 РУССКИЙ ЯЗЫК (для документации и планирования):
- **Memory Bank документы** - ВСЕ файлы в `/memory-bank/`
- **Техническая документация** - спецификации, архитектура, требования
- **Планирование задач** - tasks.md, планы, описания задач
- **Бизнес-документация** - требования, контекст, цели
- **Коммуникация в команде** - обсуждения, комментарии к задачам
- **README файлы** - описания проектов и модулей
- **Git commit messages** - сообщения коммитов
- **Пользовательская документация** - руководства, FAQ

### 🇺🇸 АНГЛИЙСКИЙ ЯЗЫК (для программного кода):
- **Весь программный код** - переменные, функции, классы, интерфейсы
- **Комментарии в коде** - inline комментарии, JSDoc, описания функций
- **API документация** - OpenAPI/Swagger, endpoint описания
- **Техническая документация кода** - README для отдельных модулей
- **Названия файлов и папок** - все файлы кода
- **Environment variables** - переменные окружения
- **Database schemas** - названия таблиц, столбцов, индексов
- **Configuration files** - конфигурационные файлы
- **Test descriptions** - описания тестов и test cases
- **Error messages** - сообщения об ошибках в системе
- **Logging messages** - логи системы

### ПРИМЕРЫ ПРИМЕНЕНИЯ:

#### ✅ Правильно - Memory Bank (Русский):
```markdown
## ЗАДАЧА: Создание сервиса аутентификации
**Описание:** Реализация JWT-based аутентификации для торговой системы
**Требования:** Поддержка multi-factor authentication

## FILE REFERENCES
### Созданные файлы:
- `/src/auth/auth.service.ts` - Основной сервис аутентификации
```

#### ✅ Правильно - Программный код (Английский):
```typescript
/**
 * Authentication service for trading system
 * Handles JWT token generation and validation
 */
@Injectable()
export class AuthService {
  /**
   * Validates user credentials and generates JWT token
   * @param email User email address
   * @param password User password
   * @returns JWT token with user claims
   */
  async login(email: string, password: string): Promise<TokenResponse> {
    // Validate user credentials against database
    const user = await this.validateUser(email, password);
    
    if (!user) {
      throw new UnauthorizedException('Invalid credentials');
    }
    
    // Generate JWT token with user claims
    return this.generateToken(user);
  }
}
```

#### ❌ Неправильно - Смешение языков:
```typescript
// НЕПРАВИЛЬНО - код на английском, но комментарии на русском
async login(email: string, password: string) {
  // Проверяем учетные данные пользователя
  const user = await this.validateUser(email, password);
}

// НЕПРАВИЛЬНО - переменные на русском
const проверенныйПользователь = await this.validateUser(email, password);
```

### ОБОСНОВАНИЕ ВЫБОРА:

#### Русский для документации:
- **Лучшее понимание** бизнес-требований и архитектурных решений
- **Точность передачи** сложных технических концепций
- **Эффективная коммуникация** в русскоязычной команде
- **Снижение барьеров** при planning и design фазах

#### Английский для кода:
- **Международные стандарты** разработки ПО
- **Лучшая интеграция** с open-source библиотеками
- **Профессиональные стандарты** индустрии
- **Возможность привлечения** международных разработчиков
- **Консистентность** с ecosystem JavaScript/TypeScript

### КОНТРОЛЬ КАЧЕСТВА:
- **Code Review** должен проверять соблюдение языковой политики
- **Automated checks** для проверки языка в комментариях кода
- **Documentation review** для проверки русскоязычной документации

### ИСКЛЮЧЕНИЯ:
- **Технические термины** можем оставлять на английском в русской документации (API, endpoint, service, etc.)
- **Названия технологий** остаются на английском (Next.js, PostgreSQL, Redis)
- **Established термины** из индустрии (bug, feature, deploy, etc.)

**Это правило является ОБЯЗАТЕЛЬНЫМ для всех участников проекта.**

## ИНФРАСТРУКТУРА ДАННЫХ DOCKER КОНТЕЙНЕРОВ

### СЛУЖЕБНЫЕ ПАПКИ ДАННЫХ
**Дата добавления:** 2025-08-27 17:10:00

#### 📁 СТРУКТУРА ДАННЫХ:

```
/data/                          # Корневая папка данных
├── mongodb/                    # MongoDB персистентные данные
├── postgres/                   # PostgreSQL персистентные данные
└── redis/                      # Redis персистентные данные
```

#### 🎯 НАЗНАЧЕНИЕ:

Эти папки служат для **персистентного хранения данных** из Docker контейнеров:

##### MongoDB данные (`./data/mongodb/`):
- **Цель:** Хранение документов, коллекций, индексов MongoDB
- **Mount point:** `/data/db` внутри MongoDB контейнера
- **Содержимое:** Market data, news, indicators, ML models
- **Важность:** КРИТИЧЕСКАЯ - исторические данные и модели

##### PostgreSQL данные (`./data/postgres/`):
- **Цель:** Хранение реляционных данных PostgreSQL
- **Mount point:** `/var/lib/postgresql/data` внутри PostgreSQL контейнера  
- **Содержимое:** Users, portfolios, orders, positions, risk data
- **Важность:** КРИТИЧЕСКАЯ - транзакционные данные

##### Redis данные (`./data/redis/`):
- **Цель:** Хранение кэша и session данных Redis
- **Mount point:** `/data` внутри Redis контейнера
- **Содержимое:** Cache, sessions, pub/sub messages
- **Важность:** СРЕДНЯЯ - восстанавливаемые данные

#### 🔧 ТЕХНИЧЕСКИЕ ХАРАКТЕРИСТИКИ:

##### Docker Compose интеграция:
```yaml
# Автоматический маппинг в docker-compose.yml
postgres:
  volumes:
    - ./data/postgres:/var/lib/postgresql/data

mongodb:
  volumes:  
    - ./data/mongodb:/data/db

redis:
  volumes:
    - ./data/redis:/data
```

##### Безопасность и доступ:
- **Локальный доступ** - только с хост-машины
- **Container isolation** - изоляция между сервисами
- **Git exclusion** - исключены из версионного контроля
- **Backup ready** - готовы для резервного копирования

#### 🚨 КРИТИЧЕСКИЕ ТРЕБОВАНИЯ:

##### ✅ Обязательные правила:
- **НЕ удалять** папки во время работы контейнеров
- **НЕ редактировать** файлы БД напрямую
- **Делать backup** перед критическими изменениями
- **Мониторить размер** папок данных

##### 📋 Рекомендации по управлению:
- Регулярные backup'ы в production
- Мониторинг дискового пространства
- Автоматическая очистка старых данных
- Encryption backup'ов для production

#### 🎯 ВАЖНОСТЬ ДЛЯ ПРОЕКТА:

Эти папки являются **ФУНДАМЕНТОМ** для:
- **Персистентности торговых данных** между перезапусками
- **Быстрого восстановления** после сбоев системы
- **Разработки и тестирования** с реальными данными
- **Backup и disaster recovery** процедур

**Критично для непрерывной работы торговой системы!**
